...
static const float radius1 = 0.71;
static const float radius2 = 0.32;

static const float2 pos1 = float2(.24, -.24);
static const float2 pos2 = float2(-.6, 0.6);
...
float3 metaspheres(float2 uv1, float2 uv2, float dist, float dist1, float dist2)
{
    ...
    return lerp(sp1, sp2, ((-saturate(dist1)) + saturate(dist2) + 1) * 0.5);
}
...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    if (1 / dist > 1)
    {
        ...
        float4 color = atmo(1 / dist, shad, 0.9, atmosphereColor == 0 ? tex2D(tex, 0.5) : atmosphereColor, atmosphereShadowColor);
		
        color *= color.a;
		
        color *= (color.r + color.g + color.b) * 0.333;
